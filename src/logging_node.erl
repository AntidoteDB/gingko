...
-record(state, {
  ...
  recovered_vector :: term()  %% This is loaded on start, storing the version vector
                                      %% of the last operation appended to this log
...
init([Partition]) ->
  ...
  logger:debug(#{action => "Initializing log", partition => Partition}),

  {ok, EnableLoggingToDisk} = application:get_env(gingko, log_persistence),
  case EnableLoggingToDisk of
    true -> logger:warning("Persistent logs not implemented yet, forcing enable_log_to_disk=false");
    _ -> logger:notice("Memory-only mode logging enabled")
  end,

%%  case open_logs(LogFile, dict:new(), OpIdTable, vectorclock:new()) of
%%    {error, Reason} ->
%%      lager:error("ERROR: opening logs, reason ~w", [Partition, Reason]),
%%      {error, Reason};
%%    {Map, MaxVector} ->
...
%%  end.
  ...
  {ok, #state{
    ...
    recovered_vector=undefined,
    enable_log_to_disk=false
  }}.
...
%%    {Map, MaxVector};
%%open_logs(LogFile, Map, ClockTable, MaxVector)->
