...
-record(state, {partition :: partition_id(),
        ...
        recovered_vector :: vectorclock:vectorclock(),  %% This is loaded on start, storing the version vector
...
-spec asyn_append(index_node(), key(), #log_operation{}, sender(), boolean()) -> ok.
asyn_append(IndexNode, Log, LogOperation, ReplyTo, SyncLog) ->
    riak_core_vnode_master:command(IndexNode,
                                   {append, Log, LogOperation, SyncLog},
                                   ReplyTo,
                                   ?LOGGING_MASTER).

%% @doc synchronous append operation payload
-spec append(key(), #log_operation{}) -> {ok, op_id()} | {error, term()}.
append(Key, LogOperation) ->
    IndexNode = get_key_partition(Key),
    riak_core_vnode_master:sync_command(IndexNode,
                                        {append, Key, LogOperation, false},
                                        ?LOGGING_MASTER,
                                        infinity).

%% @doc synchronous append operation payload
%% If enabled in antidote.hrl will ensure item is written to disk
-spec append_commit(key(), #log_operation{}) -> {ok, op_id()} | {error, term()}.
append_commit(Key, Payload) ->
    IndexNode = get_key_partition(Key),
    riak_core_vnode_master:sync_command(IndexNode,
                                        {append, Key, Payload, is_sync_log()},
                                        ?LOGGING_MASTER,
                                        infinity).
...
-spec get_up_to_time(key(), vectorclock:vectorclock(), type()) ->
         [{op_num(),clocksi_payload()}] | {error, reason()}.
get_up_to_time(Key, MaxSnapshotTime, Type) ->
    IndexNode = get_key_partition(Key),
    riak_core_vnode_master:sync_command(IndexNode,
                    {get, Key, undefined, MaxSnapshotTime, Type},
                    ?LOGGING_MASTER,
                    infinity).
...
-spec get_from_time(index_node(), key(), vectorclock:vectorclock(), type(), key()) ->
         [{op_num(),clocksi_payload()}] | {error, reason()}.
...
-spec get_range(index_node(), key(), vectorclock:vectorclock(), vectorclock:vectorclock(), type(), key()) ->
         [{op_num(),clocksi_payload()}] | {error, reason()}.
...
handle_command({append_group, LogId, LogRecordList, _IsLocal = false, Sync}, _Sender,
    ...
    {ErrorList, SuccList, UpdatedLogs} =
        lists:foldl(fun(LogRecordOrg, {AccErr, AccSucc, UpdatedLogs}) ->
                        LogRecord = check_log_record_version(LogRecordOrg),
                        case get_log_from_map(Map, Partition, LogId) of
                            {ok, Log} ->
...
handle_command({get, Key, MinSnapshotTime, MaxSnapshotTime}, _Sender,
    #state{logs_map = Map, partition = Partition} = State) ->
    case get_log_from_map(Map, Partition, Key) of
        {ok, Log} ->
            ok = disk_log:sync(Log),
...
-spec get_last_op_from_log(log_id(), disk_log:continuation() | start, cache_id(), vectorclock:vectorclock()) -> {eof, vectorclock:vectorclock()} | {error, term()}.
...
-spec get_max_op_numbers([{log_id(), #log_record{}}], cache_id(), vectorclock:vectorclock()) -> vectorclock:vectorclock().
...
-spec open_logs(string(), [preflist()], dict:dict(log_id(), disk_log:log()), cache_id(), vectorclock:vectorclock()) -> {dict:dict(log_id(), disk_log:log()), vectorclock:vectorclock()} | {error, reason()}.
open_logs(_LogFile, [], Map, _ClockTable, MaxVector) ->
    {Map, MaxVector};
open_logs(LogFile, [Next|Rest], Map, ClockTable, MaxVector)->
    PartitionList = remove_node_from_preflist(Next),
...
%%
-spec join_logs([{preflist(), term()}], fun(), term()) -> term().
join_logs([], _F, Acc) ->
    Acc;
...
%%
-spec insert_log_record(term(), log_id(), #log_record{}, boolean()) -> {ok, #op_number{}} | {error, reason()}.
...
%%
-spec remove_node_from_preflist(riak_core_apl:preflist()) -> [integer()].
