...
-module(multiple_dcs_multi_dc_SUITE).
-include("gingko.hrl").
...
blocking_test(Config) ->
    ...
    rpc:call(Node3, gingko_dc_utils, bcast_gingko_sync, [?INTER_DC_LOG, {set_ping_active, false}]),
    timer:sleep(5000),
    ...
    rpc:call(Node3, gingko_dc_utils, bcast_gingko_sync, [?INTER_DC_LOG, {set_ping_active, true}]),
    timer:sleep(5000),

    check_read_key(Node3, Key, Type, 2, CommitTime3, static, Bucket),
    ct:pal("Blocking test passed!").
...
replicated_set_test(Config) ->
    ...
    ct:pal("last commit time was ~p.", [LastCommitTime]),
